...
void main() {
  ...
  testWidgets('lists all tracks alphabetically once the scanner has data', (tester) async {
  ...
  testWidgets('shows an empty state when the library has no tracks', (tester) async {
    final scanner = scannerWithSongs([]);
    await tester.runAsync(() => scanner.scan());
  ...
  testWidgets('searching filters the list by title, case-insensitively', (tester) async {
    final scanner = scannerWithSongs([
  ...
  testWidgets('swiping a track away adds it to the queue instead of restarting playback', (tester) async {
